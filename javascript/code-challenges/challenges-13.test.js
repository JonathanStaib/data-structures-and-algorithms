...
// Solution code here...
  // return arr.reduce((longest, next) => +longest.height < +next.height ? longest : next);
  let longestStr = '';
...
const firstLetters = (arr) => {
  // Solution code here...
  let newArr = arr.map(word => word.charAt(0));
  return newArr;
};
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(cell => `${cell.substring(1, 4)}${cell.substring(6,9)}${cell.substring(10)}`);
};
